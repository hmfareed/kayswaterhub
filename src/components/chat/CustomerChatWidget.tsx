...
export function CustomerChatWidget() {
  ...
  return (
    <>
      ...
      {/* ─── Full-Screen Chat View with Dynamic Viewport Height ──────────── */}
      {isOpen && (
        <div
          style={viewportHeight ? { height: `${viewportHeight}px`, maxHeight: `${viewportHeight}px` } : undefined}
          className="fixed inset-0 h-dvh max-h-dvh z-50 flex flex-col bg-slate-50 dark:bg-neutral-950 text-slate-800 dark:text-neutral-100 overflow-hidden animate-in fade-in duration-200"
        >
          {/* Top Header */}
          <div className="shrink-0 bg-gradient-to-r from-slate-900 via-blue-950 to-indigo-950 dark:from-neutral-950 dark:via-blue-950 dark:to-neutral-900 text-white shadow-md border-b border-white/10 dark:border-neutral-800 backdrop-blur-md">
            <div className="max-w-4xl mx-auto w-full px-3.5 sm:px-6 py-2.5 sm:py-3.5 flex items-center justify-between">
              <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
                <div className="relative w-9 h-9 sm:w-10 sm:h-10 rounded-2xl bg-blue-600/40 border border-blue-400/30 flex items-center justify-center shadow-inner shrink-0">
                  <Bot className="w-4 h-4 sm:w-5 sm:h-5 text-blue-200" />
                  <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-emerald-500 border-2 border-slate-900" />
                </div>
                <div className="min-w-0">
                  <div className="flex items-center gap-1.5">
                    <h3 className="font-bold text-xs sm:text-base text-white tracking-tight truncate">Kay&apos;s Packs AI</h3>
                    <span className="text-[9px] sm:text-[10px] font-semibold uppercase tracking-wider px-1.5 py-0.5 rounded-full bg-blue-500/30 text-blue-200 border border-blue-400/20 shrink-0">
                      Smart Assistant
                    </span>
                  </div>
                  <p className="text-[10px] sm:text-[11px] text-blue-200/80 flex items-center gap-1 mt-0.5 truncate">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 shrink-0" /> Online • Live Store Connected
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button
                  ...
                  className="p-1.5 sm:p-2 rounded-xl text-blue-200 hover:text-white hover:bg-white/10 transition-colors cursor-pointer"
                  aria-label="Toggle theme"
                >
                  {isDarkMode ? <Sun className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-amber-300" /> : <Moon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-blue-200" />}
                </button>
                <button
          ...
          {itemCount > 0 && (
            <div className="shrink-0 bg-blue-50/95 dark:bg-neutral-900/95 border-b border-blue-100 dark:border-neutral-800 backdrop-blur-md">
              <div className="max-w-4xl mx-auto w-full px-3.5 sm:px-6 py-1.5 sm:py-2 flex items-center justify-between text-xs">
                ...
                <Link
                  href="/checkout"
                  onClick={closeChat}
                  className="text-[10px] sm:text-[11px] font-bold text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 flex items-center gap-1 hover:underline shrink-0"
                >
          ...
          <div className="flex-1 min-h-0 overflow-y-auto p-3 sm:p-6 space-y-4 bg-gradient-to-b from-slate-50/70 to-white dark:from-black dark:to-neutral-950 text-slate-800 dark:text-neutral-100 text-xs custom-scrollbar overscroll-contain">
            <div className="max-w-4xl mx-auto w-full space-y-4">
              {messages.map((m) => {
                const isBot = m.role === "assistant";
                return (
                  <div key={m.id} className={`flex flex-col ${isBot ? "items-start" : "items-end"} animate-fade-in`}>
                    <div className={`flex items-start gap-2 max-w-[92%] sm:max-w-[80%] ${isBot ? "" : "flex-row-reverse"}`}>
                      {isBot ? (
                        <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-blue-600 text-white flex items-center justify-center shrink-0 mt-0.5 text-[11px] sm:text-xs font-bold shadow-xs">
                          K
                        </div>
                      ) : (
                        <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-slate-700 dark:bg-neutral-800 text-white flex items-center justify-center shrink-0 mt-0.5 text-[11px] sm:text-xs">
                          <User className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                        </div>
                      )}
                      <div
                        className={`p-3 sm:p-4 rounded-2xl leading-relaxed whitespace-pre-wrap break-words text-xs sm:text-sm ${
                          isBot
                    ...
                    <span className="text-[9px] sm:text-[10px] text-slate-400 dark:text-neutral-500 mt-1 px-8 sm:px-10">{m.timestamp}</span>

                    {/* Suggested Product Cards embedded if returned */}
                    {isBot && m.suggestedProducts && m.suggestedProducts.length > 0 && (
                      <div className="mt-3 pl-8 sm:pl-10 w-full space-y-2">
                        <p className="text-[10px] sm:text-[11px] font-bold text-slate-500 dark:text-neutral-400 uppercase tracking-wider">
                          Matched Products:
                        </p>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                          {m.suggestedProducts.map((prod) => {
                            ...
                            return (
                              <div
                                key={prod.id}
                                className="flex items-center gap-3 p-2.5 sm:p-3 bg-white dark:bg-neutral-900 rounded-2xl border border-slate-200 dark:border-neutral-800 shadow-xs hover:border-blue-300 dark:hover:border-blue-700 transition-all"
                              >
                                <div className="relative w-12 h-12 sm:w-14 sm:h-14 rounded-xl bg-slate-100 dark:bg-neutral-800 flex items-center justify-center shrink-0 overflow-hidden">
                                  {prod.images && prod.images[0] ? (
                                    <Image src={prod.images[0]} alt={prod.name} fill className="object-contain p-1" />
                                  ) : (
                                    <Bot className="w-5 h-5 sm:w-6 sm:h-6 text-slate-400" />
                                  )}
                                </div>
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex items-center justify-between mt-1">
                                    ...
                                    <button
                                      onClick={() => handleProductAdd(prod, 1)}
              ...
              {isLoading && (
                <div className="flex items-center gap-2.5 text-xs text-slate-400 py-2">
                  <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-blue-600 text-white flex items-center justify-center shrink-0 text-xs font-bold animate-pulse">
                    K
                  </div>
                  <div className="bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-2xl rounded-tl-xs flex items-center gap-2 shadow-xs">
                    ...
                    <span className="text-[11px] text-slate-500 dark:text-neutral-400 font-medium ml-1">Checking store data...</span>
                  </div>
                </div>
              )}
          ...
          <div className="shrink-0 bg-white/95 dark:bg-neutral-950/95 border-t border-slate-200/80 dark:border-neutral-850 backdrop-blur-md pb-[max(0.5rem,env(safe-area-inset-bottom))]">
            {/* Quick Suggestion Chips */}
            <div className="max-w-4xl mx-auto w-full px-3 sm:px-4 pt-1.5 sm:pt-2.5 flex gap-1.5 overflow-x-auto text-[10px] sm:text-[11px] custom-scrollbar">
              {QUICK_SUGGESTION_CHIPS.map((chip, idx) => (
                <button
                  key={idx}
                  onClick={() => handleSend(chip.query)}
                  className="whitespace-nowrap px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full bg-slate-100 dark:bg-neutral-900 hover:bg-blue-50 dark:hover:bg-blue-950 hover:text-blue-600 dark:hover:text-blue-300 text-slate-700 dark:text-neutral-300 font-medium transition-colors cursor-pointer border border-transparent hover:border-blue-200 dark:hover:border-blue-800"
                >
                  {chip.label}
                </button>
              ))}
            ...
            {/* Chat Input Bar */}
            <div className="max-w-4xl mx-auto w-full p-2.5 sm:p-4 flex items-center gap-2">
              <input
                ref={inputRef}
                type="text"
                value={input}
              ...
              <button
                ...
                className="p-2.5 sm:p-3 rounded-xl bg-blue-600 hover:bg-blue-700 active:scale-95 text-white disabled:bg-slate-200 dark:disabled:bg-neutral-800 disabled:text-slate-400 dark:disabled:text-neutral-600 transition-all cursor-pointer shadow-md shadow-blue-600/20 shrink-0 flex items-center justify-center"
            ...
            <div className="max-w-4xl mx-auto w-full px-4 pb-2 sm:pb-2.5 flex items-center justify-between text-[9px] sm:text-[11px] text-slate-500 dark:text-neutral-400">
